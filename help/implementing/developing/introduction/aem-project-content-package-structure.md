...
description: Erfahren Sie, wie Sie Paketstrukturen für die Bereitstellung im Adobe Experience Manager Cloud-Dienst ordnungsgemäß definieren.
...
# Grundlegendes zur Struktur eines Projekts Content Package im Adobe Experience Manager Cloud-Dienst {#understand-cloud-service-package-structure}
...
AEM erfordert eine Trennung von **Inhalt** und **Code**. Dies bedeutet, dass ein einzelnes Inhaltspaket nicht für **sowohl** schreibbare als auch für Laufzeitbereiche bereitstellen **kann** `/apps` (z. B. `/content`, `/conf`, `/home`oder alles andere `/apps`) des Repositorys. Stattdessen muss die Anwendung Code und Inhalt in separate Pakete für die Bereitstellung in AEM trennen.

Die in diesem Dokument beschriebene Paketstruktur ist mit **lokalen Entwicklungs-Bereitstellungen und AEM Cloud-Dienstbereitstellungen kompatibel** .
...
Alles andere im Repository, `/content`, `/conf`, `/var`, `/home`, `/etc`, `/oak:index`, `/system`, `/tmp`, etc. sind alle **veränderbare** Bereiche, d.h. sie können zur Laufzeit geändert werden.
...
+ Das `ui.content` Paket bzw. das Codepaket enthält alle Inhalte und Konfigurationen. Zu den gebräuchlichen Elementen des `ui.content` Pakets gehören:
   + Kontextsensitive Konfigurationen
      + `/conf`
   + Grundlegende Inhaltsstrukturen (Asset-Ordner, Sites-Stammseiten)
   ...
   + Ältere Knoten
      + `/etc`
   + ACLs (Berechtigungen)
      + Alle `rep:policy` für einen Pfad, der **nicht** unter `/apps`
+ Das `all` Paket ist ein Container-Paket, das NUR die `ui.apps` und `ui.content` Pakete als Einbettung enthält. Das `all` Paket darf keinen eigenen Inhalt **** haben, sondern die Bereitstellung an das Repository an seine Unterpakete delegieren.
...
Adobe Cloud Manager löst standardmäßig alle Pakete aus, die vom Maven-Build erstellt wurden. Da jedoch das Container-Paket (`all`) das einzige Bereitstellungs-Artefakt ist, das alle Code- und Inhaltspakete enthält, müssen wir sicherstellen, dass **nur** das Container-Paket (`all`) bereitgestellt wird. Um dies sicherzustellen, müssen andere Pakete, die der Maven-Build generiert, mit der FileVault Content Package Maven Plug-In-Konfiguration von gekennzeichnet werden `<properties><cloudManagerTarget>none</cloudManageTarget></properties>`.
...
## Einbetten von Unterpaketen in das Containerpaket{#embeddeds}
...
Um AEM-Autor, AEM-Veröffentlichung oder beides als Ziel festzulegen, wird das Paket in das `all` Containerpaket in einem speziellen Ordner-Speicherort im folgenden Format eingebettet:
...
+ Der Ordner auf der zweiten Ebene stellt die Anwendung dar, deren Ordnername nach dem `-packages` Fixieren festgelegt ist. Oft gibt es nur einen einzigen Ordner auf 2. Ebene, in den alle Unterpakete eingebettet sind. Es können jedoch beliebig viele Ordner auf 2. Ebene erstellt werden, um die logische Struktur der Anwendung am besten darzustellen:
   ...
   >[!WARNING]
   >
   >Eingebettete Ordner mit Unterpaketen werden standardmäßig mit dem Suffix von `-packages`. Dadurch wird sichergestellt, dass der Bereitstellungscode und die Inhaltspakete **nicht** in den Zielordnern eines Unterpakets bereitgestellt werden, `/apps/<app-name>/...` was zu destruktiven und zyklischen Installationsweisen führt.
...
+ Der Ordner auf der vierten Ebene enthält die Unterpakete und muss eines der folgenden sein:
   + `install` zur Installation auf **** AEM Authoring und AEM Publishing
   + `install.author` nur **** auf AEM-Autor installieren
...
Eine Bereitstellung mit AEM Authoring- und Veröffentlichungsspezifischen Paketen könnte beispielsweise wie folgt aussehen:
...
### Filterdefinition des Containerpakets {#container-package-filter-definition}

Aufgrund der Einbettung von Code- und Inhalts-Unterpaketen in das Containerpaket müssen die eingebetteten Zielpfade zum Containerprojekt hinzugefügt werden, `filter.xml` um sicherzustellen, dass die eingebetteten Pakete beim Erstellen im Containerpaket enthalten sind.
...
Wenn sich die Pakete von Drittanbietern in einem **öffentlichen Maven-Artefaktrepository** von Drittanbietern befinden, muss dieses Repository im Projekt registriert `pom.xml` und nach der oben [beschriebenen](#embeddeds)Methode eingebettet werden. Wenn die Drittanbieteranwendung/der Drittanbieter sowohl Code- als auch Inhaltspakete erfordert, müssen alle Pakete in die richtigen Positionen im Container-(`all`) Paket eingebettet werden.
...
#### Behälterpakettypen {#container-package-types}

Das Containerprojekt `all/pom.xml` deklariert **keine** `<packageType>`.
...
Fügen Sie in jedem Projekt, das ein Paket generiert, **mit Ausnahme** des Containerprojekts (`all`), `<cloudManagerTarget>none</cloudManagerTarget>` der `<properties>` Konfiguration der `filevault-package-maven-plugin` Plug-in-Deklaration hinzu, um sicherzustellen, dass sie nicht **** von Adobe Cloud Manager bereitgestellt werden. Das Paket &quot;Container&quot;(`all`) sollte das Einzelpaket sein, das über Cloud Manager bereitgestellt wird. Dadurch werden wiederum alle erforderlichen Code- und Inhaltspakete eingebettet.
...
### Einbetten von Unterpaketen in das Containerpaket {#xml-embeddeds}
...
### Filterdefinition des Containerpakets {#xml-container-package-filters}
...
Wenn mehrere Elemente in den Einbettungszielen verwendet `/apps/*-packages` werden, müssen sie alle hier aufgezählt werden.
...
### Bereinigen des Zielordners des Containerprojekts {#xml-clean-container-package}

Fügen Sie im `all/pom.xml` Plug-in das `maven-clean-plugin` Plug-In hinzu, das den Zielordner vor einem Maven-Build bereinigt.
