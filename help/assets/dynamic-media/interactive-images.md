...
Die folgende Abbildung zeigt ein Banner mit Shopping-Funktion mit einem Schnellansichts-Popup-Fenster. Benutzer können die Schnellansicht aktivieren, indem sie auf den Kreis oder „Hotspot“ des Modells tippen.
...
1. **Hochladen eines Bild-Banners**. Laden Sie Bild-Banner hoch, die Sie interaktiv machen möchten. Siehe [Hochladen eines Bild-Banners](#uploading-an-image-banner).

1. **Hinzufügen von Hotspots zu einem Bild-Banner**. Fügen Sie einen oder mehrere Hotspots zu einem Bild-Banner hinzu. Verknüpfen Sie jeden einzelnen mit einer Aktion wie etwa einem Hyperlink, einer Schnellansicht oder einem Experience Fragment. Nachdem Sie Hotspots hinzugefügt haben, schließen Sie diese Aufgabe ab, indem Sie das interaktive Bild veröffentlichen.
...
Wenn Sie Experience Manager-Assets und Dynamic Media in der Standalone-Version verwenden, kopieren Sie den Einbettungs-Code auf Ihrer Website. Integrieren Sie ihn dann in Ihre vorhandene Schnellansicht. Siehe [Integrieren eines interaktiven Bildes auf Ihrer Website](#integrating-an-interactive-image-with-your-website).
Wenn Sie einen WCM (Web Content Manager) eines Drittanbieters verwenden, integrieren Sie das neue interaktive Video mit der auf Ihrer Website verwendeten Schnellansicht. Siehe [Integrieren eines interaktiven Bildes in einer vorhandenen Schnellansicht](#integrating-an-interactive-image-with-an-existing-quickview).
...
Im Anschluss sollten verschiedene Bereiche der bestehenden Website besucht werden, in denen die Schnellansichtsfunktion implementiert ist. Trigger dann die Schnellansicht an und ruf die Ajax-URL, die von der Webseite zum Laden der Schnellansichtsdaten oder -inhalte gesendet wird.
...
Drücken Sie bei einem Mac Befehlstaste + Wahltaste + I, um den Arbeitsbereich mit Entwickler-Tools zu öffnen, und klicken Sie dann auf die Registerkarte „Inspektor“.
...
Im einfachsten Fall ist der einzige variable Teil der Schnellansichts-URL die Produkt-SKU. In diesem Fall ist der SKU-Wert das einzige Datenteil, das Sie benötigen, um dem Banner-Bild Hotspots hinzuzufügen.
...
1. Führen Sie einen der folgenden Schritte aus:
   ...
   * Tippen Sie auf **[!UICONTROL Schnellansicht]**.
      ...
      * Wenn Sie Experience Manager Sites oder eCommerce-Kunde sind, tippen oder klicken Sie auf das Produktauswahlsymbol (Lupe), um die Seite &quot; auswählen&quot;zu öffnen. Tippen Sie auf das gewünschte Produkt und dann in der oberen rechten Ecke der Seite auf **Auswählen**. Sie gelangen zurück zur Hotspot-Verwaltungsseite.
   ...
   * Tippen Sie auf **[!UICONTROL Experience Fragment]**.
      ...
      * Wenn Sie Experience Manager Sites-Kunde sind, tippen oder klicken Sie auf das Suchsymbol (Lupe), um die Seite &quot;Experience Fragment&quot;zu öffnen. Tippen Sie auf das Experience Fragment, das Sie verwenden möchten. Tippen Sie dann in der oberen rechten Ecke der Seite auf **[!UICONTROL Auswählen]**. Sie gelangen zurück zur Hotspot-Verwaltungsseite.
Weitere Informationen finden Sie unter [Experience Fragments](/help/sites-cloud/authoring/fundamentals/experience-fragments.md).
...
Der letzte Schritt in diesem Prozess ist die Integration des interaktiven Bildes in eine vorhandene Schnellansichtsimplementierung auf Ihrer Website. Es gibt keine Lösung für die Integration, die für alle Fälle funktioniert. Jede Implementierung der Schnellfunktion ist einzigartig und erfordert eine individuelle Vorgehensweise. Daher ist es hilfreich, die Unterstützung durch eine IT-Person am Frontend einzubeziehen.
...
Der Prozess der Erstellung der Schnellansichts-URL ist das Gegenteil des Prozesses, der verwendet wird, um die zuvor erläuterten Hotspot-Variablen zu ermitteln.
...
Der letzte Schritt zum Trigger der Schnellansichts-URL und zum Aktivieren des Schnellansichtsbereichs erfordert die Unterstützung eines Frontend-IT-Mitarbeiters von Ihrer Arbeit. Sie verfügt am ehesten über das entsprechende Fachwissen, um die Schnellansichtsimplementierung im jeweiligen Schritt entsprechend auszulösen, um eine einsatzbereite Schnellansichts-URL zu erhalten.

Sie können sehen, wie diese Schritte auf die Demo-Website angewendet werden, sodass ein interaktives Bild mit Shopping-Funktion mit dem Schnellansichts-Code vollständig integriert wird. Die Struktur der Schnellansichts-URL wurde bereits wie folgt dargestellt:
