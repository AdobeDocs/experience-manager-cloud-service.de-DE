...
+++**Sind alle Assets im as a Cloud Service Repository von Experience Manager Assets für die Suche und Bereitstellung mit Dynamic Media mit OpenAPI-Funktionen verfügbar?**

Nein, nur [genehmigte und neueste Version der Assets](/help/assets/approved-assets.md) sind für die Suche und Bereitstellung mit Dynamic Media mit OpenAPI-Funktionen verfügbar, um die Markenkonsistenz über alle Kanäle und Anwendungen hinweg sicherzustellen.
...
Experience Manager Assets unterscheidet den Status Genehmigt mit ![Assets genehmigen](assets/thumbs-up-icon.svg) auf der Asset-Karte verfügbar sind, wie in der folgenden Abbildung dargestellt:

![Symbol für genehmigte Assets](/help/assets/assets/approved-assets-thumbs-up.png)

Informationen zum Genehmigen aller Assets in einem Ordner finden Sie in den Anweisungen unter [Verwalten von Massengenehmigungen für Assets in einem Ordner](/help/assets/approved-assets.md#bulk-approve-assets). Es gibt auch ein Video, das den gesamten Prozess darstellt.
...
Das Asset ist als `Rejected`, wenn der Administrator `Rejected` oder `Changes requested` -Werte. Experience Manager Assets unterscheidet den Status Abgelehnt mit ![Ablehnen von Assets](/help/assets/assets/do-not-localize/reject-assets.svg) auf der Asset-Karte verfügbar.
...
Führen Sie die folgenden Schritte aus, um mehrere Assets gleichzeitig in zu genehmigen [!DNL Experience Manager Assets]:
...
Als DAM-Benutzer verfügen Sie möglicherweise nicht über Berechtigungen für [Genehmigen von Assets](approved-assets.md#approve-assets). Um die Berechtigungen zum Bearbeiten des Genehmigungsstatus eines Assets zu erhalten, können Administratoren das standardmäßige oder jedes andere Metadatenschema bearbeiten, das auf den Asset-Ordner angewendet wird, um der **[!UICONTROL Prüfungsstatus]** -Feld. Weitere Informationen finden Sie unter [Deaktivieren der Bearbeitung für den Prüfungsstatus](approved-assets.md#configuration) -Feld.
...
|---|---|
| [Nur mit as a Cloud Service Assets verfügbar](/help/assets/new-dynamic-media-overview.md#prerequisites-new-dynaminc-media-apis) | Auch mit On-Premise oder Adobe Managed Services mit zusätzlichen Konfigurations- und Bereitstellungsschritten verfügbar. |
...
| Stack basierend auf OpenAPI-Spezifikationen, mit denen die meisten Entwickler übereinstimmen. Die AEM Assets-Erweiterung wird durch die Verwendung von [Micro Frontend-Asset-Auswahl](/help/assets/asset-selector.md). | SOAP-basierte APIs, die bei der Entwicklung von Integrationsanpassungen zu einer Barriere werden. |
...
| Nur genehmigte Assets sind für die Asset-Bereitstellung an nachgelagerte Anwendungen verfügbar, sodass für markengenehmigte Assets digitale Erlebnisse aktiviert werden. Bereitgestellte Assets berücksichtigen den Asset-Ablaufstatus in der -Autoreninstanz AEM as a Cloud Service Repositorys. | Alle Aktualisierungen eines von Dynamic Media veröffentlichten Assets werden ohne Genehmigungs-Workflow automatisch veröffentlicht, was in digitalen Erlebnissen nicht für markengenehmigte Assets sorgt. Kein inhärentes Asset-Ablaufdatum. Ein Asset bleibt öffentlich, bis es aus AEM as a Cloud Service Repository gelöscht wird. |
...
|---|---|
| Assets in der Remote-DAM-Bereitstellung sind auf AEM as a Cloud Service verfügbar. | Assets in der Remote-DAM-Bereitstellung können auf AEM as a Cloud Service oder Adobe-Managed Services verfügbar sein. |
