...
|--- |--- |--- |--- |--- |
| `ref` | fragmentSelectorRef | Nein | | Verweis auf die `ContentFragmentSelector`-Instanz, die den Zugriff auf bereitgestellte Funktionen wie `reload` ermöglicht. |
...
| `getImsToken` | Ruft den `imsToken` für den aktuell angemeldeten Benutzer ab, der zum Authentifizieren von Anforderungen an andere Services verwendet werden kann, z. B. zum Generieren von Asset-Ausgabedarstellungen. |
