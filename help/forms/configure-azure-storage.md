...
[[!DNL Experience Manager Forms] Data Integration](data-integration.md) bietet eine [!DNL Azure]-Speicherkonfiguration, um Formulare mit [!DNL Azure]-Speicher-Services zu integrieren. Das Formulardatenmodell (FDM) kann verwendet werden, um adaptive Forms zu erstellen, die mit [!DNL Azure] -Server, um geschäftliche Workflows zu aktivieren. Zum Beispiel:

* Schreiben von Daten in [!DNL Azure] bei Übermittlung von adaptiven Formularen.
* Daten schreiben in [!DNL Azure] durch benutzerdefinierte Entitäten, die im Formulardatenmodell (FDM) definiert sind, und umgekehrt.
...
Nachdem Sie die [!DNL Azure]-Speicherkonfiguration erstellt haben, können Sie [das Formulardatenmodell erstellen](create-form-data-models.md). Geben Sie den Ordner an, der die [!DNL Azure] Konfiguration in der **[!UICONTROL Datenquellenkonfiguration]** beim Erstellen des Formulardatenmodells (FDM). Anschließend können Sie die Konfiguration aus der Liste der Konfigurationen auswählen, die im angegebenen Ordnernamen vorhanden sind.

### Hinzufügen von [!DNL Azure]-Services zum Formulardatenmodell {#add-azure-services}

Nachdem Sie das Formulardatenmodell (FDM) und die Datenmodellobjekte erstellt haben, können Sie [!DNL Azure] Dienste für das Formulardatenmodell (FDM).

So fügen Sie [!DNL Azure]-Services hinzu:

1. Wählen Sie im Bearbeitungsmodus die Services im Abschnitt **[!UICONTROL Services]** im linken Bereich aus und wählen Sie dann **[!UICONTROL Auswahl hinzufügen]**. Die ausgewählten Dienste werden im **[!UICONTROL Dienste]** Registerkarte des Formulardatenmodells (FDM).
...
1. Auswählen **[!UICONTROL Speichern]** , um das Formulardatenmodell (FDM) zu speichern.
...
1. Auswählen **[!UICONTROL Fertig]** und wählen Sie **[!UICONTROL Speichern]** , um das Formulardatenmodell (FDM) zu speichern.
