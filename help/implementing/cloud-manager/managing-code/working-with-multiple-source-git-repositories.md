...
1. Die Aktion checkt Projekt A und das Cloud Manager-Git-Repository aus und kopiert alle Inhalte aus Projekt A in den Ordner `project-a` im Git-Repository von Cloud Manager.
1. Dann wird die Änderung durch die Aktion commits-pusht.

Beispielsweise wird eine Änderung an der Hauptverzweigung in Projekt A automatisch an die Hauptverzweigung im Git-Repository von Cloud Manager übergeben. Es kann eine Zuordnung zwischen Verzweigungen wie Push-Benachrichtigungen zu einer Verzweigung mit dem Namen geben `dev` in Projekt A an eine Verzweigung mit dem Namen `development` im Git-Repository von Cloud Manager. Ähnliche Schritte sind für Projekt B erforderlich.

Je nach Verzweigungsstrategie und Workflows kann die Synchronisierung für verschiedene Verzweigungen konfiguriert werden. Wenn das verwendete Git-Repository kein Konzept bereitstellt, das GitHub-Aktionen ähnelt, ist auch eine Integration über Jenkins (oder Ähnliches) möglich. In diesem Fall löst ein Webhook einen Jenkins-Vorgang aus, der die Arbeit ausführt.

Führen Sie diese Schritte aus, damit Sie eine neue, dritte Quelle oder ein neues Repository hinzufügen können.

1. Fügen Sie dem neuen Repository eine GitHub-Aktion hinzu, durch die Änderungen aus diesem Repository in das Git-Repository von Cloud Manager übertragen werden.
...
>[!NOTE]
>
>Das Beispielskript verwendet `git add`, um das Repository zu aktualisieren. Dabei wird davon ausgegangen, dass Entfernungen enthalten sind. Abhängig von der Standardkonfiguration von Git muss dies durch `git add --all`.
