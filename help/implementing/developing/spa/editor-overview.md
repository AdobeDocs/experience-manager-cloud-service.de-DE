...
* [Erste Schritte mit SPA in AEM mithilfe von React](getting-started-react.md) für eine schnelle Einführung in eine einfache SPA mithilfe von React
* [Erste Schritte mit SPA in AEM mit Angular](getting-started-angular.md) für eine schnelle Einführung in eine einfache SPA mit Angular
...
### Arbeitsablauf für SPA Editor {#basic-spa-editor-workflow}
...
Damit der Autor den Seiteneditor zum Bearbeiten des Inhalts eines SPA verwenden kann, muss Ihre SPA Anwendung implementiert sein, um mit dem AEM SPA Editor SDK zu interagieren. Bitte lesen Sie das [Erste Schritte mit SPA in AEM React](getting-started-react.md) Dokument für ein Minimum, das Sie wissen müssen, um Ihre Arbeit zu starten.
...
Zusätzliche SPA können implementiert werden, um mit dem AEM SPA Editor SDK zu arbeiten. Im Dokument [SPA Blueprint](blueprint.md) finden Sie Informationen zu den Anforderungen, die ein Framework erfüllen muss, um eine Framework-spezifische Ebene aus Modulen, Komponenten und Diensten zu erstellen, die mit dem AEM SPA Editor verwendet werden kann.
...
1. Legen Sie ein Attribut (das beliebig sein kann) für das Container-Wrapper-Element fest, das die Text-HTML enthält. Im Falle des WKND SPA Projekts ist es ein `<div>` Element und der Selektor, der verwendet wurde, ist `data-rte-editelement`.
