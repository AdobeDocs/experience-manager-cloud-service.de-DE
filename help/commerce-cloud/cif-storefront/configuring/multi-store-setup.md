...
1. Konfigurieren Sie mehrere Stores und Store-Ansichten nach dem Muster, das unter [Websites, Stores und Ansichten in Adobe Commerce beschrieben ist.](https://experienceleague.adobe.com/docs/commerce-admin/start/setup/websites-stores-views.html?lang=de)

1. Stellen Sie sicher, dass die Verbindung zwischen AEM und Adobe Commerce funktioniert.

1. Erstellen Sie eine untergeordnete Konfiguration der CIF-Cloud Service-Konfiguration wie folgt:

   * Navigieren Sie in AEM zu Tools > Allgemein [Konfigurationsbrowser.](/help/implementing/developing/introduction/configurations.md#using-configuration-browser)
...
1. Weisen Sie die untergeordnete Konfiguration einer AEM-Site zu.

   * Wechseln Sie zur AEM Sites-Konsole.
   * Navigieren Sie zum Regions- oder Sprachstamm Ihrer Site .structure. Beispiel: `/content/venia/us _or_ /content/venia/us/en` für die Venia-Beispielseite.
   * Wählen Sie die Seite aus und öffnen Sie die Seiteneigenschaften.
   * Wählen Sie die Registerkarte Erweitert aus.
   * Wählen Sie im Abschnitt `Configuration` die Konfiguration aus, die Sie in Schritt 3 erstellt haben.
