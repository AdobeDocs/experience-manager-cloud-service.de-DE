...
1. Geben Sie im Dialogfeld **Validierung der Eigentümerschaft eines privaten Repositorys** ein Zugriffs-Token an, um die Eigentümerschaft des externen Repositorys zu validieren, sodass Sie darauf zugreifen können.
   ...
   *Auswählen eines vorhandenen Zugriffs-Tokens für ein Bitbucket-Repository.*
   ...
   | **Neues Zugriffs-Token hinzufügen** | **Repository-Typ: GitHub Enterprise**<br><ul><li> Geben Sie **Textfeld &quot;**&quot; einen Namen für das Zugriffstoken ein, das Sie erstellen.<li>Erstellen Sie ein persönliches Zugriffstoken, indem Sie die Anweisungen in der [GitHub-Dokumentation](https://docs.github.com/en/enterprise-server@3.14/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) befolgen.<li>Erforderlich Berechtigungen für das GitHub Enterprise Personal Access Token (PAT)<br>Diese Berechtigungen stellen sicher, dass Cloud Manager abrufen Anfragen validieren, managen Statusprüfungen festschreiben und auf die erforderlichen Repo-Details zugreifen kann.<br>Wenn Sie den Pfad in GitHub Enterprise generieren, stellen Sie sicher, dass er die folgenden Repository-Berechtigungen enthält:<ul><li>Pull-Anforderung (Lesen und Schreiben)<li>Commit-Status (Lesen und Schreiben)<li>Repository-Metadaten (schreibgeschützt)</li></li></ul></li></ul></ul></ul><ul><li>Fügen Sie im Feld **Zugriffs** Token das soeben erstellte Token ein. |
   | | **Repository-Typ: GitLab**<ul><li>Geben Sie im **Textfeld &quot;Token** Name&quot; einen Namen für die Zugriffstoken ein, die Sie erstellen.<li>Erstellen eine persönliche Zugriffstoken, indem Sie den Anweisungen in der [GitLab-Dokumentation](https://docs.gitlab.com/user/profile/personal_access_tokens/) folgen.<li>Erforderlich Berechtigungen für das GitLab Personal Access Token (PAT)<br>Diese Bereiche ermöglichen Cloud Manager den Zugriff auf Repository Daten und User Informationen, die für die Tauglichkeitsprüfung- und Webhook-Integration erforderlich sind.<br>Wenn Sie den Pfad in GitLab generieren, stellen Sie sicher, dass er die folgenden Token-Bereiche enthält:<ul><li>API<li>read_user</li></li></ul></li></li></ul></ul></ul><ul><li>Fügen Sie im **Feld &quot;Access Token** &quot; das soeben erstellte Token ein. |
   | | **Repository-Typ: Bitbucket**<ul><li>Geben Sie im **Textfeld &quot;Token** Name&quot; einen Namen für die Zugriffstoken ein, die Sie erstellen.<li>Erstellen einen Repository Zugriffstoken mithilfe der [Bitbucket-Dokumentation](https://support.atlassian.com/bitbucket-cloud/docs/create-a-repository-access-token/).<li>Erforderlich Berechtigungen für das Bitbucket Personal Access Token (PAT)<br>Mit diesen Berechtigungen kann Cloud Manager auf Repository Inhalte zugreifen, Anfragen managen abrufen und Webhook-Ereignisse konfigurieren oder darauf reagieren.<br>Wenn du die App-Kennwort in Bitbucket erstellst, stelle sicher, dass sie die folgenden erforderlichen App-Kennwort Berechtigungen enthält:<ul><li>Repository (schreibgeschützt)<li>Pull-Anforderungen (Lesen und Schreiben)<li>Webhooks (Lesen und Schreiben)</li></li></ul></li></li></ul></ul></ul><ul><li>Fügen Sie im **Feld &quot;Access Token** &quot; das soeben erstellte Token ein. |
...
Die Webhook-Konfiguration ist für Repositorys, die auf `GitHub.com` gehostet werden, nicht erforderlich, da Cloud Manager direkt über die GitHub-App integriert wird.

Für alle anderen externen Repositorys, die mit einem Zugriffstoken integriert sind, z. B. GitHub Enterprise, GitLab und Bitbucket, ist die Webhook-Konfiguration verfügbar und muss manuell eingerichtet werden.
...
1. Konfigurieren Sie den Webhook so, dass die erforderlichen Ereignisse gesendet werden, die von Cloud Manager erwartet werden.
   ...
   | --- | --- |
   | GitHub Enterprise | Diese Ereignisse ermöglichen es Cloud Manager, auf GitHub-Aktivität zu reagieren, z. B. abrufen Anfrage Tauglichkeitsprüfung, Push-basierte Trigger für Pipelines oder Edge Lieferung Services Code-Synchronisation.<br>Stellen Sie sicher, dass der Webhook so eingerichtet ist, dass er bei den folgenden erforderlichen Webhook-Ereignissen ausgelöst wird:<ul><li>Pull-Anfragen<li>Schiebt<li>Problem Kommentare</li></li></li></ul></ul></ul> |
   | GitLab | Diese Webhook-Ereignisse ermöglichen es Cloud Manager, Pipelines beim Pushen von Trigger oder bei der Übermittlung einer Zusammenführungsanfrage zu konfigurieren. Sie verfolgen auch Kommentare im Zusammenhang mit der Validierung von Pull-Anforderungen (durch Notizereignisse).<br>Stellen Sie sicher, dass der Webhook für die folgenden erforderlichen Webhook-Ereignisse auf Trigger eingerichtet ist<ul><li>Ereignisse pushen<li>Zusammenführen Anfrage Ereignisse<li>Notiz-Ereignisse</li></li></li></ul></ul></ul> |
   | Bitbucket | Diese Ereignisse stellen sicher, dass Cloud Manager Pull-Anforderungen validieren, auf Code-Push-Benachrichtigungen reagieren und mit Kommentaren zur Pipeline-Koordinierung interagieren kann.<br>Stellen Sie sicher, dass der Webhook für die folgenden erforderlichen Webhook-Ereignisse auf Trigger eingerichtet ist<ul><li>Pull-Anfrage: Erstellt<li>Pull-Anfrage: Aktualisiert<li>Pull-Anforderungen: Zusammengeführt<li>Pull Anfrage: Kommentieren<li>Repository: Push</li></li></li></ul></ul></ul> |
