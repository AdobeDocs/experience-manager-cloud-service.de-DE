...
* Bei einigen Benutzern treten möglicherweise Probleme beim Erstellen von Formularen auf `Cloud Service version 2024.5.16461`Das Problem besteht darin, dass beim Erstellen eines Formulars die folgende Fehlermeldung im Erstellungsdialogfeld angezeigt wird:

  `A server error occurred. Try again after sometime.`

  Das Problem tritt auf, weil der Autor das Formular veröffentlicht, ohne zuerst die im Formular verwendete Vorlage zu veröffentlichen. Dies führt zum Hinzufügen der `jcr:uuid` und anderen geschützten und systemgenerierten Eigenschaften an die `<template-path>/initial/jcr:content` Knoten. Dies führt zu Fehlern bei der nachfolgenden Formularerstellung.

  Führen Sie die angegebenen Schritte aus, um das Problem zu beheben:

   1. Stellen Sie sicher, dass die Vorlage, die Sie im Formular verwenden, nicht über die `jcr:uuid` und anderen systemgenerierten geschützten Eigenschaften im Pfad `<template-path>/initial/jcr:content` Knoten.
