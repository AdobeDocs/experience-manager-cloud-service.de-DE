...
#### Wie bei Standard-Assets wird das Inhaltsfragment gespeichert in:{#asset-location}
...
Funktionsintegration {#feature-integration}](/help/assets/content-fragments/content-fragments-delete.md)

#### Integration mit dem Assets-Kern:{#feature-integration}
...
>Die [Inhaltsfragment-Komponente als Teil der Kernkomponenten[#$tu71]. Weitere Informationen finden Sie unter [Entwickeln von Kernkomponenten[#$tu72].
>
>



* Mit der Komponente können Sie die Variante auswählen, die angezeigt werden soll.`fragmentPath``fragmentPath`
...
### Inhaltsfragmente können mit folgenden Frameworks integriert werden:{#integration-with-other-frameworks}
...
* **Inhaltsfragmente sind vollständig mit dem AEM-Übersetzungs-Workflow integriert. Auf Architekturebene bedeutet dies:**
   ...
   * sie befinden sich unter verschiedenen Sprachstämmen, verwenden aber genau denselben relativen Pfad unterhalb des entsprechenden Sprachstamms:
      ...
      * `/content/dam/<path>/en/<to>/<fragment>`
         ...
         `/content/dam/<path>/en/<to>/<fragment>`im Vergleich zu

         `/content/dam/<path>/de/<to>/<fragment>`

         `/content/dam/<path>/de/<to>/<fragment>`Außer den regelbasierten Pfaden besteht keinerlei Verbindung zwischen den unterschiedlichen Sprachversionen von Inhaltsfragmenten. Sie werden als zwei separate Fragmente behandelt, obwohl die Benutzeroberfläche Funktionen zum Navigieren zwischen den Sprachvarianten beinhaltet.
   * [!NOTE]
...
* Inhaltsfragmente verwenden [Metadatenschemata](/help/assets/metadata-schemas.md) (wieder), die mit Standard-Assets definiert werden können.
   ...
   * CFM bietet ein eigenes, spezifisches Schema:[](/help/assets/metadata-schemas.md)

   * `/libs/dam/content/schemaeditors/forms/contentfragment`

      `/libs/dam/content/schemaeditors/forms/contentfragment`dieses kann bei Bedarf erweitert werden.
...
## Sie können die serverseitige API für den Zugriff auf Inhaltsfragmente verwenden, siehe:{#the-content-fragment-management-api-server-side}

[com.adobe.cq.dam.cfm[#$tu111]



>[!CAUTION]Es wird dringend empfohlen, die serverseitige API zu verwenden, anstatt direkt auf die Inhaltsstruktur zuzugreifen.
>
>Hauptschnittstellen     {#key-interfaces}

### Die folgenden drei Schnittstellen können als Einstiegspunkte dienen:{#key-interfaces}

**Inhaltsfragment** ([ContentFragment[#$tu117])
...
* 
   ...
   * Auflisten von Elementen
      ...
      * Zugriff auf Elementdaten (siehe `ContentElement`)](#caveats)

      * Auflisten der für das Fragment definierten Varianten`ContentElement`
   ...
   **Inhaltselement** ([ContentElement[#$tu136])
   ...
   * 
      ...
      * Auflisten von Varianten
         ...
         * Zugriff auf Variantendaten (siehe `ContentVariation`)](#caveats)
         * Tastaturbefehl zum Auflösen von Varianten (Anwenden zusätzlicher implementierungsspezifischer Ausweich-Logik, falls die angegebene Variante für ein Element nicht verfügbar ist)`ContentVariation`
      * **Inhaltsvariante** ([ContentVariation[#$tu147])
   * 
   ...
   Erstellen neuer Versionen des Elements`ContentFragment``ContentElement``ContentVariation``Versionable`
...
### Folgendes kann angepasst werden:{#adapting-using-adaptto}
...
* `Resource`: die zugrunde liegende Sling-Ressource. Um die zugrunde liegende `Resource` direkt zu aktualisieren, muss das `ContentFragment`-Objekt neu erstellt werden.
   ...
   * `ContentElement` kann angepasst werden an:`Asset``Asset``ContentFragment`
...
### Beachten Sie Folgendes:{#caveats}
...
* Erstellen Sie neue Varianten von `ContentFragment`, um die Datenstruktur zu aktualisieren.
   ...
   * Client-seitige API für die Inhaltsfragmentverwaltung     {#the-content-fragment-management-api-client-side}`ContentFragment.removeVariation()`

## [!CAUTION]

>[!CAUTION]Die Client-seitige API ist intern.
>
>Zusätzliche Informationen {#additional-information}

### Beachten Sie Folgendes:{#additional-information}
...
### Dazu können Sie die Ressource, die für die API steht, wie folgt anpassen:{#example-accessing-an-existing-content-fragment}

`com.adobe.cq.dam.cfm.ContentFragment`

`com.adobe.cq.dam.cfm.ContentFragment`Beispiel:
...
Knoten: `<conf-root>/settings/dam/cfm/jcr:content`](/help/assets/content-fragments/content-fragments-managing.md#save-cancel-and-versions)

* Eigenschaftsname: `autoSaveInterval`
* Typ: `Long`
* Standard: `600` (10 Minuten); wird definiert in `/libs/settings/dam/cfm/jcr:content`

* Wenn Sie ein Intervall von 5 Minuten für das automatische Speichern festlegen möchten, müssen Sie die Eigenschaft auf dem Knoten definieren. Beispiel:`600``/libs/settings/dam/cfm/jcr:content`
...
## Weitere Informationen finden Sie unter{#components-for-page-authoring}

[Kernkomponenten – Inhaltsfragmentkomponente[#$tu206] (empfohlen)
