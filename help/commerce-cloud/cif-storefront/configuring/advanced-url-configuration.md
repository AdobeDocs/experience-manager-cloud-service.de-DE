...
>[!NOTE]
>
> Suchmaschinenoptimierung (SEO) ist zu einem wichtigen Thema für viele Marketer geworden. Daher müssen SEO-Themen bei vielen Projekten in Adobe Experience Manager (AEM) as a Cloud Service berücksichtigt werden. Weitere Informationen finden Sie unter [Best Practices für SEO und URL-Verwaltung](/help/overview/seo-and-url-management.md).
...
Um den `UrlProvider`-Service entsprechend den SEO-Anforderungen und -Anforderungen zu konfigurieren, muss ein Projekt eine OSGi-Konfiguration für die _CIF URL Provider-Konfiguration bereitstellen_.
...
Es ist möglich, [Mehrkategorien- und Produktseiten](/help/commerce-cloud/cif-storefront/authoring/multi-template-usage.md) nur für eine bestimmte Teilmenge von Kategorien oder Produkten eines Katalogs zu erstellen.
...
#### Verwenden Sie ein URL-Format für die Produktseite, das die SKU enthält. {#use-sku}
...
#### Verwenden Sie ein URL-Format für die Produktseite, das den Kategoriekontext enthält. {#use-url}
...
#### Schaffen Sie ein Gleichgewicht zwischen URL-Länge und kodierten Informationen. {#balance-url}
