---
title: Hinzufügen von externen Repositorys in Cloud Manager – begrenzte Beta-Version
...
1. Klicken Sie im Seitenmenü unter **Programm** auf ![Ordnergliederungssymbol](https://spectrum.adobe.com/static/icons/workflow_18/Smock_FolderOutline_18_N.svg) **Repositorys**.
...
1. Geben Sie in jedem Feld jeweils die folgenden Details zu Ihrem Repository an:
   ...
   | **Repository-Typ auswählen** | Erforderlich. Wählen Sie den verwendeten Repository-Typ aus:<ul><li>**GitHub** (GitHub Enterprise Server und die selbst gehostete GitHub-Version)</li><li>**GitLab** (sowohl `gitlab.com` als auch die selbst gehostete GitLab-Version) </li><li>**Bitbucket** (sowohl `bitbucket.org` als auch Bitbucket Server und die selbst gehostete Bitbucket-Version)</li></ul>Wenn der obige Repository-URL-Pfad den Namen des Git-Anbieters enthält, z. B. GitLab oder Bitbucket, ist der Repository-Typ bereits für Sie vorausgewählt. |
...
1. Geben Sie im Dialogfeld **Validierung der Eigentümerschaft eines privaten Repositorys** ein Zugriffs-Token an, um die Eigentümerschaft des externen Repositorys zu validieren, sodass Sie darauf zugreifen können.
   ...
   *Auswählen eines vorhandenen Zugriffs-Tokens für ein Bitbucket-Repository.*
   ...
   | **Neues Zugriffs-Token hinzufügen** | **Repository-Typ: GitHub**<br><ul><li> Geben Sie **Textfeld &quot;**&quot; einen Namen für das Zugriffstoken ein, das Sie erstellen.<li>Erstellen Sie ein persönliches Zugriffstoken, indem Sie die Anweisungen in der [GitHub-Dokumentation](https://docs.github.com/en/enterprise-server@3.14/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) befolgen.<li>Legen Sie für die erforderlichen Berechtigungen Folgendes fest:<ul><li>**Erforderliche Berechtigungen für das GitHub Personal Access Token (PAT)**<br> Diese Berechtigungen stellen sicher, dass Cloud Manager Pull-Anforderungen validieren, Commit-Statusprüfungen verwalten und auf erforderliche Repo-Details zugreifen kann. Wenn Sie in GitHub das persönliche Zugriffstoken (PAT) generieren, stellen Sie sicher, dass es Folgendes enthält:<ul><li>**Repository-Berechtigungen:**<ul><li>Pull-Anforderung (Lesen und Schreiben)<li>Commit-Status (Lesen und Schreiben)<li>Repository-Metadaten (schreibgeschützt)</li></li></ul></ul><li>**Erforderliche Webhook-Ereignisse (für GitHub-gehostete Repositorys)**<br> Diese Ereignisse ermöglichen es Cloud Manager, auf GitHub-Aktivitäten zu reagieren, z. B. die Validierung von Pull-Anforderungen, Push-basierte Trigger für Pipelines oder Edge Delivery Services-Codesynchronisierung. Wenn Sie einen GitHub-Webhook manuell konfigurieren, stellen Sie sicher, dass der Webhook für die folgenden Ereignisse auf Trigger eingerichtet ist:<ul><li>**Erforderliche Webhook-Ereignisse:**<ol><li>Pull-Anforderungen<li>Push-Benachrichtigungen<li>Anfragekommentare</li></li></ul></ul><ul><li>Fügen Sie im Feld **Zugriffs** Token das soeben erstellte Token ein. |
   |  | **Repository-Typ: GitLab**<ul><li>Geben Sie **Textfeld &quot;**&quot; einen Namen für das Zugriffstoken ein, das Sie erstellen.<li>Erstellen Sie ein persönliches Zugriffstoken, indem Sie den Anweisungen in der [GitLab-Dokumentation](https://docs.gitlab.com/user/profile/personal_access_tokens/) folgen.<li>Legen Sie für die erforderlichen Berechtigungen Folgendes fest:<ul><li>**Erforderliche Berechtigungen für das GitLab Personal Access Token (PAT)**<br> Diese Bereiche ermöglichen Cloud Manager den Zugriff auf Repository-Daten und Benutzerinformationen, die für die Validierung und Webhook-Integration erforderlich sind. Stellen Sie beim Erstellen des persönlichen Zugriffstokens in GitLab sicher, dass es die folgenden Bereiche enthält:<ul><li>**Token-Bereiche:**<ul><li>API<li>read_user</li></li></ul></ul><li>**Erforderliche Webhook-Ereignisse (für von GitLab gehostete Repositorys)**<br> Diese Webhook-Ereignisse ermöglichen es Cloud Manager, Pipelines beim Pushen von Trigger oder beim Senden einer Zusammenführungsanfrage zu konfigurieren. Sie verfolgen auch Kommentare im Zusammenhang mit der Validierung von Pull-Anforderungen (durch Notizereignisse). Wenn Sie einen Webhook in GitLab manuell konfigurieren, stellen Sie sicher, dass Sie die folgenden Ereignisse einschließen:<ul><li>**Erforderliche Webhook-Ereignisse:**<ol><li>Push-Ereignisse<li>Zusammenführen von Anfrageereignissen<li>Notiz-Ereignisse</li></li></li></ul></ul></ul><ul><li>Fügen Sie im Feld **Zugriffs** Token das soeben erstellte Token ein. |
   |  | **Repository-Typ: Bitbucket**<ul><li>Geben Sie **Textfeld &quot;**&quot; einen Namen für das Zugriffstoken ein, das Sie erstellen.<li>Erstellen Sie ein Repository-Zugriffstoken mithilfe der [Bitbucket-Dokumentation](https://support.atlassian.com/bitbucket-cloud/docs/create-a-repository-access-token/).<li>Legen Sie für die erforderlichen Berechtigungen Folgendes fest:<ul><li>**Erforderliche Berechtigungen für den Bitbucket Personal Access Token (PAT)**<br> Diese Berechtigungen ermöglichen Cloud Manager den Zugriff auf Repository-Inhalte, die Verwaltung von Pull-Anfragen und die Konfiguration von oder Reaktion auf Webhook-Ereignisse. Stellen Sie beim Erstellen des App-Kennworts in Bitbucket sicher, dass es Folgendes enthält:<ul><li>**Erforderliche App-Passwortberechtigungen:**<ul><li>Repository (schreibgeschützt)<li>Pull-Anforderungen (Lesen und Schreiben)<li>Webhooks (Lesen und Schreiben)</li></li></ul></ul><li>**Erforderliche Webhook-Ereignisse (für durch Bitbuckets gehostete Repositorys)**<br> Diese Ereignisse stellen sicher, dass Cloud Manager Pull-Anforderungen validieren, auf Code-Push-Benachrichtigungen reagieren und mit Kommentaren für die Pipeline-Koordinierung interagieren kann. Wenn Sie den Webhook manuell in Bitbucket einrichten, konfigurieren Sie ihn für die folgenden Ereignisse für den Trigger:<ul><li>**Erforderliche Webhook-Ereignisse:**<ol><li>Pull-Anfrage: Erstellt<li>Pull-Anfrage: Aktualisiert<li>Pull-Anforderungen: Zusammengeführt<li>Pull-Anforderung: Kommentar<li>Repository: Push</li></li></li></ul></ul></ul><ul><li>Fügen Sie im Feld **Zugriffs** Token das soeben erstellte Token ein. |
...
Für alle anderen externen Repositorys, die mit einem Zugriffs-Token integriert sind, wie GitHub Enterprise Server, GitLab und Bitbucket, ist eine Webhook-Konfiguration verfügbar, die manuell eingerichtet werden muss.
...
* **GitHub Enterprise Server**
  ...
  Wenn die Prüfung erstellt wird, sieht sie wie der folgende Screenshot aus. Der wesentliche Unterschied zu `GitHub.com` besteht darin, dass `GitHub.com` eine Überprüfungsausführung verwendet, während GitHub Enterprise Server (unter Verwendung von persönlichen Zugriffstoken) einen Commit-Status generiert:

  ![Commit-Status zur Angabe des PR-Validierungsprozesses in GitHub Enterprise Server](/help/implementing/cloud-manager/managing-code/assets/repository-webhook-github-pr-validation.png)
