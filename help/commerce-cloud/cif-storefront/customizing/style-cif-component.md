...
Das [CIF Venia](https://github.com/adobe/aem-cif-guides-venia)Projekt ist eine Referenz-Code-Basis für die Verwendung von [CIF-Kernkomponenten.](https://github.com/adobe/aem-core-cif-components) In diesem Tutorial sehen Sie sich das Venia-Referenzprojekt an und erfahren, wie CSS und JavaScript, die von AEM CIF-Kernkomponenten verwendet werden, organisiert sind. Außerdem erstellen Sie mithilfe von CSS einen Stil, um den Standardstil der Komponente **Produkt-Teaser** zu aktualisieren.
...
1. Überprüfen Sie anschließend die anderen vom Projekt erstellten Client-Bibliotheken:
   ...
   **clientlib-base** - Eine leere Client-Bibliothek, die einfach die erforderlichen Abhängigkeiten von [AEM-Kernkomponenten einbettet.](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=de) Die Kategorie ist `venia.base`.

   **clientlib-cif** - Eine leere Client-Bibliothek, die einfach die erforderlichen Abhängigkeiten von [AEM CIF-Kernkomponenten einbettet.](https://github.com/adobe/aem-core-cif-components) Die Kategorie ist `venia.cif`.
...
Es gibt mehrere Optionen zum Einschließen einer Client-seitigen Bibliothek. Überprüfen Sie als Nächstes, wie das generierte Projekt die `clientlib-site` Bibliotheken über [Seitenvorlagen.](/help/implementing/developing/components/templates.md)
...
1. Öffnen Sie ein neues Terminal-Fenster und navigieren Sie zum Ordner `ui.frontend`. Führen Sie den Befehl `npm start` aus:
   ...
   Dieser Befehl startet den webpack-dev-server auf [http://localhost:8080/.](http://localhost:8080/)
...
1. Kehren Sie zu Ihrem Browser zurück und aktualisieren Sie die Homepage: [http://localhost:4502/editor.html/content/venia/us/en.html.](http://localhost:4502/editor.html/content/venia/us/en.html) Der aktualisierte Produkt-Teaser-Stil sollte angezeigt werden.
...
Außerdem versucht AEM, Client-Bibliotheken für höhere Leistung zwischenzuspeichern. Gelegentlich werden nach einer Code-Implementierung die älteren Dateien bereitgestellt. Sie können den Client-Bibliotheks-Cache von AEM mithilfe des Tools [Client-Bibliotheken neu erstellen“ manuell ungültig machen.](http://localhost:4502/libs/granite/ui/content/dumplibs.rebuild.html) *Invalidieren von Caches ist die bevorzugte Methode, wenn Sie vermuten, dass AEM eine alte Version einer Client-Bibliothek zwischengespeichert hat. Die Neuerstellung von Bibliotheken ist ineffizient und zeitaufwendig.*
